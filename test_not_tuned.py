...
def main():
    ...
    with open('params_not_tuned.json', 'r') as f:
        best_params = json.load(f)
    print(f"Loaded not-tuned parameters: {best_params}")

    # 2. Train Model on 2023-2025
    print("\n--- Training Model with Not-Tuned Parameters ---")
    try:
    ...
    print("\n--- Not-Tuned OOS Results ---")
    backtester.analyze(results)
    
    # Plot
    plt.figure(figsize=(10, 6))
    plt.plot(results.index, results['equity'], label='Not-Tuned Strategy Equity')
    plt.plot(results.index, (1 + results['market_returns']).cumprod() * 500_000_000, label='Market (VN30F)')
    plt.title("Not-Tuned OOS Strategy - 2026")
    plt.legend()
    plt.savefig("not_tuned_oos_2026.png")
    print("\nEquity curve saved to 'not_tuned_oos_2026.png'")
